...
logs.logFiles = function(dir,fileName,callback){
  fileName = fileName.trim();
  return new Promise(function(resolve,reject){
    dataLib.read(dir,fileName,function(err,data){
      console.log('called',fileName,'.json',data,)
      if (!err){
        //'users' + '-' +email+ '-' + Date.now()
        logs.createFile(dir + '-' + fileName + '-' + Date.now(),'.json',data,function(err){
          if (!err){
            resolve(callback(false))
          }else{
            reject(callback(err))
...
logs.createFile = function(fileName,format,string,callback){
  fs.open(logs._baseDir+'/'+fileName+format,'wx',function(err,fileDiscriptor){
