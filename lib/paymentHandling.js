...
payment.gui.pay = async function(data,callback){
  _helpers.getTemplate('payment',function(err,dashData){
    if (!err && dashData){
      let tableHTMLstring = '';
      let renderInfo = {
        '{head.title}' : 'payment',
        '{body.table}' : tableHTMLstring
      }
...
payment.gui.success = function(data,callback){
  _helpers.getTemplate('success',function(err,dashData){
    if (!err && dashData){
      let tableHTMLstring = '';
      let renderInfo = {
        '{head.title}' : 'success',
...
payment.pay = async function(data,callback){
  ...
  if (method){
    await payment.pay[data.method](data,function(statusCode,err,type){
...
payment.pay.post = function(data,callback){
  ...
  let cardNumber = typeof(parseInt(data.payload.cardNumber)) == 'number' ? parseInt(data.payload.cardNumber) : false;
  let csv = typeof(parseInt(data.payload.csv)) == 'number' ? parseInt(data.payload.csv) : false;
  let stripeToken = config.stripeToken;
  console.log(data.payload,token,orderId,firstName,lastName,email,cardNumber,csv,stripeToken)
////////////////
...
//////////
      ...
      return new Promise(function(resolve,reject){
              ...
              let req = https.request(options,function(res){
                if (res.statusCode == 200 || res.statusCode == 201){
                  console.log(res.statusCode,'success trans')
                  resolve(data);
                }else {
                  console.log(res)
  ...
  .then(function(data){
    ...
    resolve(callback(200,'paid','plain'))
    })
  })

  .catch((error) => {
      console.log("ghfhf",error)
        callback(error[0],error[1],'JSON')
      })
